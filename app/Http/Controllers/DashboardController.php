...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $dataServiceTotal = Service::where('created_at', date('Y-m-d'))->count();
        $dataServiceHandphone = Service::where('type', '2')->where('created_at', date('Y-m-d'))->count();
        $dataServiceLaptop = Service::where('type', '3')->where('created_at', date('Y-m-d'))->count();
