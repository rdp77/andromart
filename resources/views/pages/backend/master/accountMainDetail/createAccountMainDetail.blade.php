...
@section('title', __('pages.title').__(' | Tambah Master Akun Detail'))
@section('titleContent', __('Tambah Master Akun Detail'))
@section('breadcrumb', __('Data'))
@section('morebreadcrumb')
<div class="breadcrumb-item active">{{ __('Master Akun Detail') }}</div>
<div class="breadcrumb-item active">{{ __('Tambah Master Akun Detail') }}</div>
@endsection

@section('content')
<div class="card">
    <form method="POST" action="{{ route('account-main-detail.store') }}">
        @csrf
        <div class="card-body">
            <div class="row">
                <div class="form-group col-md-3 col-xs-12">
                    ...
                    <select class="form-control select2" name="main_id" id="main_id" required>
                        ...
                        <option value="{{ $main->id}}"> {{ $main->code }} - {{ $main->name }} </option>
                        @endforeach
                    </select>
                </div>
